...
public class SetTablesServlet extends HttpServlet {
	...
	public void tabWineGres(HttpServletRequest request,
		...
		if ("wineGreBig".equals(wineGreType)) {
			...
			LayuiJSON<WineGreBig> layuiJSON = DBUtil.layuiReturn(null,
					totalRow, wineGreBigs);
			DBUtil.jsonObjectReturn(response, layuiJSON);
		} else if ("wineGreSmall".equals(wineGreType)) {
			int wineGreBigId = Integer.parseInt(request.getParameter(
					"wineGreBigId").toString());
			List<WineGreSmall> wineGreSmalls = wineService.findSmalll(page,
					wineGreBigId);
			long totalRow = wineService.getTotalRows(wineGreBigId);
			if (wineGreSmalls.size() == 0 && totalRow == 0) {
			...
			LayuiJSON<WineGreSmall> layuiJSON = new LayuiJSON<WineGreSmall>(0,
					"", totalRow, wineGreSmalls);
			DBUtil.jsonObjectReturn(response, layuiJSON);
		} else if ("wineGre".equals(wineGreType)) {
			int wineGreSmallId = Integer.parseInt(request.getParameter(
					"wineGreSmallId").toString());
			List<WineGre> wineGres = wineService
					.selectAll(page, wineGreSmallId);
			long totalRow = wineService.getTotalRow(wineGreSmallId);
			if (wineGres.size() == 0 && totalRow == 0) {
			...
			LayuiJSON<WineGre> layuiJSON = new LayuiJSON<WineGre>(0, "",
					totalRow, wineGres);
			DBUtil.jsonObjectReturn(response, layuiJSON);
		}
	}
	...
	public void addEidtSave(HttpServletRequest request,
		...
		if ("wineGreBig".equals(wineGreType)) {
			WineGreBig wineGreBig = RequestHelper.getSingleRequest(request,
					WineGreBig.class);
			...
			WineGreSmall wineGreSmall = RequestHelper.getSingleRequest(request,
					WineGreSmall.class);
	...
	public void findTypeById(HttpServletRequest request,
		...
		String wineGreType = request.getParameter("wineGreType");
		if ("wineGreBig".equals(wineGreType)) {
			int id = Integer.parseInt(request.getParameter("wineGreBigId")
					.toString());
			WineGreBig wineGreBig = wineService.findByID(id);
			DBUtil.jsonObjectReturn(response, wineGreBig);
		} else if ("wineGreSmall".equals(wineGreType)) {
			int id = Integer.parseInt(request.getParameter("wineGreSmallId")
					.toString());
			WineGreSmall wineGreSmall = wineService.findBYID(id);
			DBUtil.jsonObjectReturn(response, wineGreSmall);
		} else if ("wineGre".equals(wineGreType)) {
			int id = Integer.parseInt(request.getParameter("wineGreId")
					.toString());
			WineGre wineGre = wineService.findById(id);
			DBUtil.jsonObjectReturn(response, wineGre);
		}
	}

	public void delWineGreType(HttpServletRequest request,
			HttpServletResponse response) throws ServletException, IOException {
		JsonReturn jsonReturn = new JsonReturn();
		String wineGreType = request.getParameter("wineGreType");
		String str = new String();
	...
	public void getNumber(HttpServletRequest request,
		...
		if ("wineGreBig".equals(wineGreType)) {
			...
			if (wineGreBigs.size() != 0) {
				num = wineGreBigs.get(wineGreBigs.size() - 1)
						.getWineGreBigNum();
			...
			if (wineGreSmalls.size() != 0) {
				num = wineGreSmalls.get(wineGreSmalls.size() - 1)
						.getWineGreSmallNum();
				num = String.valueOf(Integer.parseInt(num) + 1);
			} else {
				num = String.valueOf(1);
	...
	public void addEidtWineGre(HttpServletRequest request,
			HttpServletResponse response) throws ServletException, IOException {
		JsonReturn jsonReturn = new JsonReturn();
		WineGre wineGre = new WineGre();
	...
	public void rtPicture(HttpServletRequest request,
			HttpServletResponse response) throws ServletException, IOException {
